...
async def push_close_display(_: None = Depends(require_operator)):
    ...
    for rig in enrolled:
        ...
        if not (status and getattr(status, "online", False)):
            continue
        agent_ids.append(agent_id)
        tasks.append(send_agent_command(agent_id, "close-display", {}, timeout=15.0))
    if not tasks:
        return {"ok": True, "results": [], "message": "No online agents found"}
    raw_results = await asyncio.gather(*tasks, return_exceptions=True)
    results = []
...
async def get_sim_state(agent_id: str):
    ...
    if agent_status:
        ...
        if getattr(agent_status, "race_session_type", None) and isinstance(agent_status.race_session_type, str):
            payload["race_session_type"] = agent_status.race_session_type
        if getattr(agent_status, "race_total_laps", None) is not None:
...
async def launch_online(agent_id: str, request: LaunchOnlineRequest, _: None = Depends(require_operator)):
    ...
    try:
        ...
        if server_id:
            fav = _get_favourite_by_id(server_id)
            if fav:
                snapshot = _build_favourite_server_cfg_snapshot(fav, server_id, skip_live_fetch=False)
                srv = (snapshot or {}).get("SERVER") or {}
                resolved_track = (srv.get("TRACK") or "").strip()
                if not resolved_track:
                    raise _launch_online_validation_error(
                        400,
                        "track_unresolvable",
                        f"Cannot join server: unable to resolve track/layout metadata from {server_ip}:{server_port_int}. "
                        "The server may be offline or not responding to info queries.",
                        hint="Ensure the server is online and reachable. Try again in a few seconds.",
                    )
                body["server_cfg_snapshot"] = snapshot
        ...
        if resolved_host is not None and resolved_port is not None:
            ...
            if fav:
                ...
                if not resolved_track:
                    raise HTTPException(
                        status_code=400,
                        detail=f"Cannot resolve track/layout metadata for server {server_id}. The server may be offline.",
                    )
                body["server_cfg_snapshot"] = snapshot
                body["preset_name"] = server_id
            else:
